...
def connection_p(A,B,ax=None,**kwargs):
    if ax == None:
        ax = plt.gca()
    line = lines.Line2D([A[0],B[0]], [A[1],B[1]], axes=ax,**kwargs)
    ax.add_line(line)

def connection_xy(X,Y,ax=None,**kwargs):
    if ax == None:
        ax = plt.gca()
...
### CURVES ###
# Draw an open curve
def curve_xy(x,y,ax=None,**kwargs):
    if ax == None:
        ax = plt.gca()
    line = lines.Line2D(x,y, axes=ax,**kwargs)
    ax.add_line(line)

def curve_p(P,ax=None,**kwargs):
    if ax == None:
        ax = plt.gca()
    x,y = points_to_xy(P)
    line = lines.Line2D(x,y, axes=ax,**kwargs)
...
def closed_curve_xy(x,y,ax=None,**kwargs):
    if ax == None:
        ax = plt.gca()
...
def closed_curve_p(P,ax=None,**kwargs):
    x,y = points_to_xy(P)
    closed_curve_xy(x,y,ax,**kwargs)
...
### CIRCLES ###
def circle_xy(x,y,r,ax=None,**kwargs):
    if ax == None:
        ax = plt.gca()
...
def circle_p(p,r,ax=None,**kwargs):
    if ax == None:
        ax = plt.gca()
...
def circles_xy(X,Y,R,ax=None,**kwargs):
    if ax == None:
        ax = plt.gca()
    circles = []
...
def circles_p(P,R,ax=None,**kwargs):
    if ax == None:
        ax = plt.gca()
    circles = []
    for p,r in zip(P,R):  
...
### RECTANGLE ###
def rect_xy(x0,y0,x1,y1,ax=None,**kwargs):
    if ax == None:
        ax = plt.gca()
...
### MATHEMATICAL PLOTS ###
# Scatter plots
def dots_xy(x,y,ax=None,**kwargs):
    if ax == None:
        ax = plt.gca()
    ax.scatter(x,y,**kwargs)

def dots_p(P,ax=None,**kwargs):
    if ax == None:
        ax = plt.gca()
    x,y = points_to_xy(P)
    ax.scatter(x,y,**kwargs)
